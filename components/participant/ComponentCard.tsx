...
const ComponentCard: React.FC<ComponentCardProps> = ({ component, cart, addToCart, updateCartItemQuantity }) => {
  ...
  const getStatus = () => {
    if (component.hasQuantityLimit === false) {
      return { text: 'Available', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' };
    }
  ...
  // For unlimited components, there's no max limit. For limited components, use available quantity.
  const maxAllowed = component.hasQuantityLimit === false ? Infinity : availableQuantity;

  const handleAddToCart = () => {
    if (component.hasQuantityLimit === false || maxAllowed > 0) {
      addToCart(component.id, 1);
    }
  };

  const increment = () => {
    // For unlimited components, always allow increment. For limited, check against maxAllowed.
    if (component.hasQuantityLimit === false || quantityInCart < maxAllowed) {
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4 py-4 border-t border-white/5">
          <div>
            ...
            <p className={`font-mono font-black text-2xl sm:text-3xl text-white leading-none ${highlightClass}`}>
              {component.hasQuantityLimit === false ? '∞' : availableQuantity}
            </p>
          </div>
          <div className="text-right">
            <p className="text-[9px] uppercase tracking-widest text-gray-500 font-bold mb-1">Status</p>
            <p className="font-bold text-sm text-gray-400">
              {component.hasQuantityLimit === false
                ? 'Unlimited'
      ...
      <div className="mt-4 pt-4 border-t border-white/5">
        {!isInCart ? (
          <Button
            ...
            disabled={component.hasQuantityLimit !== false && maxAllowed <= 0}
            className="w-full font-black shadow-lg"
            variant={(component.hasQuantityLimit !== false && maxAllowed <= 0) ? 'secondary' : 'primary'}
          >
            {(component.hasQuantityLimit !== false && maxAllowed <= 0) ? 'Unavailable' : 'Add to Cart'}
          </Button>
        ) : (
          <div className="flex items-center justify-between bg-black/40 border border-amber-500/30 rounded-lg p-1.5 backdrop-blur-sm">
            ...
            <button
              onClick={increment}
              disabled={component.hasQuantityLimit !== false && quantityInCart >= maxAllowed}
